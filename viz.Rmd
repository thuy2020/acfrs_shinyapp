...
acfrs %>% select(state, name, category, total_liabilities, bonds_outstanding, net_pension_liability, compensated_absences, revenues) %>% 
  rename("Total Liabilities" = total_liabilities, "Bond Oustanding" = bonds_outstanding, 
  ...
  pivot_longer(cols = 4:8,
               names_to = "indicator",
               values_to = "value") %>% 
  filter(name == "State of Arizona") -> d_barchart


means_generalpurpose <- tibble(acfrs %>% filter(category == "General Purpose") %>% 
  summarise(mean(total_liabilities), 
            mean(bonds_outstanding), 
            mean(net_pension_liability), 
            mean(compensated_absences), 
            mean(revenues)) 
  ) %>% 
...
means_generalpurpose_chart <- means_generalpurpose %>% 
  ggplot(aes(indicator, value)) +
  geom_col(fill = color) +
  scale_y_continuous(labels = scales::dollar_format()) +
  theme_minimal() +
  labs(title = "Average Values in General Purpose Category",
    x = "Indicator", 
    y = "Value"
  ) +
...
```{r}
  ...
  d_barchart %>% select(indicator, value) %>% 
  ggplot(aes(indicator, value)) +
  geom_col(fill = color) +
  scale_y_continuous(labels = scales::dollar_format()) +
  theme_minimal() +
